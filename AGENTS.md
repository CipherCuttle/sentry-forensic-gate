...
Universal Dev Spine entrypoint: run `mise run context` first when `mise` is available. The machine-readable active state packet is `docs/agent-packets/HISTORICAL_FULL_REPLAY_AUTHORIZATION_R1.json`. It is a navigation/state artifact only and must not broaden repository authority. If it says a decision is open, do not run the full historical replay until the packet explicitly records an authorized decision. Use `mise run pack` only from a clean worktree; it emits a SHA-bound context pack from the checked-out files and fails closed when local changes are present.
