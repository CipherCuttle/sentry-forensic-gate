...
5. Optionally time a limited number of calls to the existing read-only `ViemPonsV2LaunchAdapter.catchUp` **only after the reference HTTP source observes that same launch**, avoiding a source-dependent WS-first verification bias, as a *separate downstream materialization sample*. That is not full E0 entry planning or preflight. No private keys, wallet clients, signing, transaction construction, stateful recovery, or funding. No keys/endpoint URLs printed.
