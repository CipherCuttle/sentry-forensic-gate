...
| H8 | HIGH | An old historical zero-event/non-native scan could be published with a fresh current capture time because it has only a ten-minute operational expiry. This could fabricate a continuous prospective denominator without making any trade. | FIXED: enforce five-minute maximum freshness of the scanned-through canonical block even for empty scans and require both chain point and capture time before origin+seven days; keep the independent zero-event seal requirement. |\n| H9 | HIGH | A batch can correctly hash the approved origin value while canonical RPC origins disagree. | FIXED: explicitly check the activation-origin block/hash against the independently supplied block-point evidence, and reject an origin-hash drift before processing events. The independent upstream RPC proof is still an OPEN source-acquisition blocker. |\n| M1 | MEDIUM | Real factory may repeatedly emit a token across multiple batches. Immediate predecessor verification alone cannot prove historical global token uniqueness or all earlier gap-free factory events. | OPEN activation blocker if de-duplication is part of intended population definition; a complete replayable, witnessed denominator and scope-specific identity policy must be frozen before enrollment. |
