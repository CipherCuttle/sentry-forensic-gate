...
export async function assertCanaryEntryApprovalMatchesParentBuy(
  ...
  if (getAddress(intent.buyTokenOut) !== getAddress(expected.buyTokenOut)) throw new Error('CANARY_ENTRY_APPROVAL_PARENT_BINDING_TOKEN_OUT_MISMATCH');
  if (intent.buyFee !== expected.buyFee) throw new Error('CANARY_ENTRY_APPROVAL_PARENT_BINDING_FEE_MISMATCH');
