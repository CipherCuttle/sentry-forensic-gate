...
const ALLOWED_TRANSITIONS: Readonly<Record<PonsE2RecoveryStatus, readonly PonsE2RecoveryStatus[]>> =
  Object.freeze({
    ...
    TOKEN_REVOKE_INCLUDED: ['ABORTED_CLEAN', 'COMPLETED'],
    ABORTED_CLEAN: [],
    COMPLETED: []
  });
...
export function assertPonsE2StateMayStart(
  ...
  if (!existing) return;
  if (existing.status === 'COMPLETED' || existing.status === 'ABORTED_CLEAN') {
    throw new Error(
      `PONS_E2_TERMINAL_STATE_USE_NEW_STATE_PATH:${existing.status}`
    );
  }
  throw new Error(
