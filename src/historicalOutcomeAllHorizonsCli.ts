...
const OUTPUT_PATH = resolve(process.env.OUTPUT_PATH ?? 'historical-outcome-all-horizons-r1-live-receipt.json');
...
const EXPECTED_OUTCOMES = EXPECTED_REPRESENTATIVES * EXPECTED_HORIZONS.length;

if (process.env.SHADOW_ONLY !== 'true') {
  throw new Error('HISTORICAL_OUTCOME_ALL_HORIZONS_R1_REQUIRES_SHADOW_ONLY_TRUE');
}
if (JSON.stringify(OUTCOME_HORIZONS) !== JSON.stringify(EXPECTED_HORIZONS)) {
  throw new Error(`HISTORICAL_OUTCOME_ALL_HORIZONS_R1_HORIZON_DRIFT:${JSON.stringify(OUTCOME_HORIZONS)}`);
}
...
if (fixture.representatives.length !== EXPECTED_REPRESENTATIVES) {
  throw new Error(`HISTORICAL_OUTCOME_ALL_HORIZONS_R1_REPRESENTATIVE_COUNT:${fixture.representatives.length}`);
}
...
if (headBlock < OUTCOME_CONFIRMATIONS) {
  throw new Error(`HISTORICAL_OUTCOME_ALL_HORIZONS_R1_HEAD_TOO_LOW:${headBlock}`);
}
...
for (const representative of fixture.representatives) {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `HISTORICAL_OUTCOME_ALL_HORIZONS_R1_LAUNCH_IDENTITY_COUNT:block=${launchBlock}:tokenId=${tokenId}:count=${matches.length}`
    );
  }
  ...
  const outcomeSource = new HistoricalRedstoneOutcomeSource({ rpcUrl, authority });
  for (const horizon of OUTCOME_HORIZONS) {
    ...
    if (outcome.policyVersion !== HISTORICAL_FORWARD_OUTCOMES_REDSTONE_ASOF_R1) {
      throw new Error(`HISTORICAL_OUTCOME_ALL_HORIZONS_R1_POLICY_DRIFT:${outcome.policyVersion}`);
    }
    if (outcome.horizonMs !== horizon.ms) {
      throw new Error(
        `HISTORICAL_OUTCOME_ALL_HORIZONS_R1_HORIZON_RESULT_DRIFT:expected=${horizon.ms}:actual=${outcome.horizonMs}`
      );
    }
    ...
    outcomeRows.push({
      ...
      ...(outcome.valuationEvidence ? { valuationEvidence: outcome.valuationEvidence } : {})
    });
...
const outcomePass =
  baselinePass &&
  outcomesAttempted === EXPECTED_OUTCOMES &&
  outcomesComplete === EXPECTED_OUTCOMES &&
  outcomesUnverified === 0 &&
  outcomeRows.length === EXPECTED_OUTCOMES;

const receipt = {
  schema: 'historical-outcome-all-horizons-r1-live-receipt/v1',
  phase: 'HISTORICAL_OUTCOME_ALL_HORIZONS_R1',
  stage: 'ALL_HORIZON_REPRESENTATIVE_GATE',
  sourceMode: 'LIVE_ARCHIVE_RPC_EXISTING_HISTORICAL_BASELINE_AND_OUTCOME_PIPELINES',
  shadowOnly: true,
  ...
  expectedOutcomeCount: EXPECTED_OUTCOMES,
  baselineAttempted,
  baselineComplete,
  baselineUnverified,
  outcomesAttempted,
  ...
  baselineRows,
  outcomeRows,
  verdict: outcomePass ? 'HISTORICAL_OUTCOME_ALL_HORIZONS_R1_PASS' : 'HISTORICAL_OUTCOME_ALL_HORIZONS_R1_FAIL'
};
...
function parseFixture(value: unknown): Fixture {
  if (!value || typeof value !== 'object') throw new Error('HISTORICAL_OUTCOME_ALL_HORIZONS_R1_FIXTURE_INVALID');
  const record = value as Partial<Fixture>;
  if (record.schema !== 'historical-compatibility-r1-fixtures/v1') {
    throw new Error(`HISTORICAL_OUTCOME_ALL_HORIZONS_R1_FIXTURE_SCHEMA:${String(record.schema)}`);
  }
  if (!record.historical_executable_tuple || !Array.isArray(record.representatives)) {
    throw new Error('HISTORICAL_OUTCOME_ALL_HORIZONS_R1_FIXTURE_INCOMPLETE');
  }
...
function makeScopedAuthorizer(launchBlock: bigint, decisionBlock: bigint): ExecutableBlockAuthorizer {
  ...
  return (blockNumber: bigint): void => {
    if (!allowed.has(blockNumber.toString())) {
      throw new Error(
        `HISTORICAL_OUTCOME_ALL_HORIZONS_R1_SCOPE_VIOLATION:block=${blockNumber}:launch=${launchBlock}:decision=${decisionBlock}`
      );
    }
  };
}
