...
if (process.env.SHADOW_ONLY !== 'true') {
  throw new Error('HISTORICAL_COMPATIBILITY_REQUIRES_SHADOW_ONLY_TRUE');
}
...
if (fixture.representatives.length !== EXPECTED_REPRESENTATIVES) {
  throw new Error(`HISTORICAL_COMPATIBILITY_REPRESENTATIVE_COUNT:${fixture.representatives.length}`);
}
...
if (headBlock < OUTCOME_CONFIRMATIONS) throw new Error(`HISTORICAL_COMPATIBILITY_HEAD_TOO_LOW:${headBlock}`);
...
for (const representative of fixture.representatives) {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `HISTORICAL_COMPATIBILITY_LAUNCH_IDENTITY_COUNT:block=${launchBlock}:tokenId=${tokenId}:count=${matches.length}`
    );
  }
...
const receipt = {
  ...
  phase: 'HISTORICAL_COMPATIBILITY_R1',
  sourceMode: 'LIVE_ARCHIVE_RPC_EXISTING_PIPELINE',
  shadowOnly: true,
...
function parseFixture(value: unknown): Fixture {
  if (!value || typeof value !== 'object') throw new Error('HISTORICAL_COMPATIBILITY_FIXTURE_INVALID');
  const record = value as Partial<Fixture>;
  if (record.schema !== 'historical-compatibility-r1-fixtures/v1') {
    throw new Error(`HISTORICAL_COMPATIBILITY_FIXTURE_SCHEMA:${String(record.schema)}`);
  }
  if (!record.historical_executable_tuple || !Array.isArray(record.representatives)) {
    throw new Error('HISTORICAL_COMPATIBILITY_FIXTURE_INCOMPLETE');
  }
...
function makeScopedAuthorizer(launchBlock: bigint, decisionBlock: bigint): ExecutableBlockAuthorizer {
  ...
  return (blockNumber: bigint): void => {
    if (!allowed.has(blockNumber.toString())) {
      throw new Error(
        `HISTORICAL_COMPATIBILITY_SCOPE_VIOLATION:block=${blockNumber}:launch=${launchBlock}:decision=${decisionBlock}`
      );
    }
  };
}
