...
const fakeAuthorized=structuredClone(packet); fakeAuthorized.state='HISTORICAL_FULL_REPLAY_AUTHORIZED'; fakeAuthorized.authorization.full_147_replay=true; fakeAuthorized.authority.historical_authorization_granted=true; fakeAuthorized.next_action='OPEN_HISTORICAL_FULL_REPLAY_R1_IMPLEMENTATION'; assert.throws(()=>validatePacket(fakeAuthorized),/authorized decision basis drift/);
const validAuthorized=structuredClone(fakeAuthorized); validAuthorized.decision_result={status:'AUTHORIZE',basis_predecessor_head:'28e598d738c633b0211bae914b8f92450ae61f90',scope_identity_sha256:'b5184624928e9dc36fd75558bf599074e9eb67e03658fe9b3f3e00668b5b8211',rationale:'Frozen scope verified; open only the separate research replay implementation.'}; validatePacket(validAuthorized);
